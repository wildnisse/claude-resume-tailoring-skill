...
## Informal / announcement voice

LinkedIn posts, job announcements, personal intros, and other own-name social artifacts. The rules above still apply (banned phrases, em-dashes, mechanical AI tells), but the register is different, and several "polish" instincts that are correct for a cover letter are wrong here. Distilled from the Titan announcement session (2026-09); the as-posted sample lives in the user repo at `job-applications/apeiron-vp-engineering-2026/titan-linkedin-announcement.md`. Read it before drafting an informal artifact; never reuse its sentences.

- **No pitch skeleton.** Hook, stakes, product claim, hiring CTA — a post where every paragraph closes an argument reads as marketing, however plain the sentences. Structure that survived review: the news stated flat, the thing described through personal curiosity ("I didn't know you could do that until I met this team"), what my part is, how it feels, soft close. Humility out-pulls product claims with a technical audience.
- **No punchline codas.** Do not land paragraphs on quotable beats ("Good problem to have.") or workshopped compressions ("turning waveforms into answers"). All were rejected in review as "nobody talks like that." Feelings get stated flat instead: "I find that stuff fascinating." "I'm loving it!"
- **Real enthusiasm is the register, stated in plain words.** Exclamation marks are fine here. Colloquial intensifiers are the actual voice: "geeky as all hell," "a ton of potential," "really interesting" (even twice in one post — a human repeating a plain word is not a tell; a model recycling a flagship phrase is). Enthusiasm still never arrives via "thrilled," "amazing," or the banned excited/passionate family.
- **Humor is welcome and load-bearing.** Puns in escalating variety (three or four across a post, never the same one twice), pop-culture nods the target audience will catch ("one ping only"), self-aware whimsy ("for lack of a more technical term: Gremlins!"). A geeky in-joke does more voice work than any crafted sentence.
- **Texture beats polish.** Mid-sentence parenthetical asides ("we use them plenty elsewhere"), sentence fragments as follow-ons ("Which is part of what makes this really interesting."), casual two-dot ellipses (".."). Fix genuine misspellings and broken grammar; leave the looseness alone. Symmetry and tidy rhythm are what make a post smell generated.
- **Facts stay canonical even when the tone is loose.** Company stage, customer status ("working with," not "already using," until deals close), and comparative claims against competing tech (only if already in the company's public marketing). The Numbers rules do not relax because the artifact is casual.
